...
It sells physical goods, so an order is not finished when it is paid — it has to
be shipped. Shipping is configured rather than universal: carriers are set up per
market, with their own prices and delays, and a customer only sees what applies
where they are.

Reference data — the catalogue, the carriers, the suppliers — is not authored in
the shop. It originates in the ERP and reaches the shop through an integration
that reconciles the two, so the shop holds a copy of something decided upstream.
The shop is the system of record for what customers actually did: carts, orders,
payments, addresses.

## The systems, and how they relate

Four, and they do not resemble each other. **They are not four views of one
thing.** One is upstream of another; one measures browsers rather than records
and therefore counts a different population; one holds only what the services
wrote down. Where they disagree, the reason is usually which of them a fact
belongs to.

Each has a skill in `.agents/skills/` telling you how to reach it, what its
contract is, and where it is known to mislead. Read the relevant skill before
calling a system.
...
- **logs-loki**: what the running services wrote, including the integration that
  carries master data between them. A service that is working writes nothing
  alarming, so silence here is not evidence that nothing changed.
- **erp-feed**: the files the company's master data arrives in, before the shop
  ever sees them. Over SFTP, with no contract at all, and no history: each file
  is the current state, and the only trace that one changed is its modification
  time.
