"""Who Philip is: a desk, and a choice of two loops.

Everything else — the ticket, the events, the record, the envelope, the bus
mount — is `core`. What is left here is the part that makes this employee
a distinct one: a directory of skills and API documentation, and the fact that it
can be driven by either of two CLIs so the loop stays a variable under test.

This is also the only place that knows Philip holds two drivers. Code that
branched on a harness name anywhere else would quietly become a third harness.
"""
...
"""The loops Philip can be driven by, and the only place that knows there are two.

A mapping rather than an `if`, so an unrecognised name is refused instead of
falling through to whichever branch happens to be last. `PHILIP_HARNESS=claude`
