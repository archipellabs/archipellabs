"""Does a skill actually get Philip into a system?
...
from roles.philip.identity import AGENT, DESK, build
...
def harness(request: pytest.FixtureRequest) -> str:
    ...
    Not `PHILIP_HARNESS`, which ran whichever one happened to be configured and
...
def ask(question: str, harness_name: str) -> dict[str, str]:
    ...
    Built through `src.identity`, which is the one place that knows Philip has
    two loops. Reaching into `core.harness.codex` here would work and would
    be a second such place — the shape the mapping in `identity` exists to stop.
    """
    import asyncio
    import tempfile
...
def test_the_desk_carries_what_the_brief_promises() -> None:
    ...
    Plus one skill that is not a system. `workspace-analysis` is the method —
    fetch to a file, check the shape, compute, verify the join — and it is the
    whole difference between this desk and Ethan's. Listed exactly rather than
    with a subset check, because a desk that silently loses it becomes Ethan
    while still being reported under Philip's name, and the campaign comparing
    the two would then be comparing nothing.

    `data/` is asserted here too: the brief and every worked example write into
    it, and `curl -o` into a missing directory fails as a local file error in the
    middle of a remote call.
    """
    import tempfile
