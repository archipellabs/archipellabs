...
A campaign over a contaminated desk measures the desk. The 3/3 that Philip scored
...
The machinery is `core.harness.desk`; the desk itself is Philip's, and it
is the desk that is on trial here. Nothing below reaches for the shared module's
own paths, because the shared module has none: `DESK.root` is where this
employee's material lives, and a test that asked the library where the desk was
would be testing every employee's desk at once, which is to say none of them.
"""
...
FORBIDDEN = {
    ...
    # words. A campaign then ran over it: 89% of the runs pulled one of these
    # strings into context, and the runs that did scored 76% against 62% for the
    # runs that did not. Each phrase below is quoted from the desk as it shipped
    # that day. Found on Ethan's desk, checked here too: the two desks are the
    # comparison, so a blacklist that guards only one of them measures the guard.
...
def test_the_blacklist_would_actually_fire() -> None:
    ...
    Wrapped on purpose. A blacklist checked against unwrapped text is a blacklist
    checked against text the desk never contains, and `flat()` is the only reason
    these match at all.
    """
    was_shipped = (
...
def test_the_child_environment_is_built_not_inherited() -> None:
    """Philip's own machinery must not reach a shell the model writes.
    ...
    Philip's own action stream — the investigated system driving the
    investigator.

    Asked of **Philip's** desk rather than of the builder in the abstract. The
    ...
    os.environ["PHILIP_HARNESS"] = "codex"
    ...
    assert not [k for k in given if k.startswith("PHILIP_")]
