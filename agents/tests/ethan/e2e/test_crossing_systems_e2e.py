...
that does `ctx.call("ethan.investigate", ...)` against a running Ethan, so the
...
def ethan_is_serving() -> object:
    """Ethan, running as its own process for the whole session.
    ...
    process = subprocess.Popen(
        ...
        cwd=str(AGENTS),
        env={**os.environ, "AGENT_NAME": "ethan"},
    ...
    while time.monotonic() < deadline:
        if process.poll() is not None:
            raise RuntimeError(f"ethan exited: {(process.stderr.read() or '')[-400:]}")
        ...
        process.kill()
        raise TimeoutError("ethan did not register ethan.investigate within 30s")
    yield process
    process.terminate()
    try:
...
def _action_stream_exists() -> bool:
    ...
    prefix = f"{namespace}:" if namespace else ""
        ...
        ["docker", "exec", "redis", "redis-cli", "EXISTS",
         f"{prefix}act:ethan.investigate"],
...
def test_carriers_cross_the_erp_feed_and_the_shop(ethan_is_serving: object) -> None:
...
def test_the_feed_grain_is_a_carrier_per_country(ethan_is_serving: object) -> None:
